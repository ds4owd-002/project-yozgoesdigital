...
### trucks
The raw table is read and with the nessescary steps cleaned and stored as trucks for further analysis.
```{r}
trucks <- trucks_raw |>
  mutate(numberplate = as.character(numberplate)) |>
...
In order generate the star schema the code below can be executed. This code will basically do a left_join for of each dimension table to the trips fact table. This generates a big table with 45 variables and 5653 tuples (or rows).
...
```
Looking at the distribution a large variance can be observed, but also different clusters of sizes can be distinguished. By grouping on typical sizes it can be expected different types transport are linked. For examples of different sizes also refer to @linda_strande_faecal_2014. To summarise the results @fig-distribution-size-bp shows the results in a boxplot and @tbl-trucksize_summary shows some of the statistics.
```{r}
...
- Time \[\] time of delivery \[\] does time affect dislodge location
